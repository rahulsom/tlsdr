...
func DetectProblem(connection Connection, alert int) {
	if alert == close_notify {
		...
		switch alert {
		case unexpected_message: process_unexpected_message(connection)
		case bad_record_mac: process_bad_record_mac(connection)
		case decryption_failed_RESERVED: process_decryption_failed_RESERVED(connection)
		case record_overflow: process_record_overflow(connection)
		case decompression_failure: process_decompression_failure(connection)
		case handshake_failure: process_handshake_failure(connection)
		case no_certificate_RESERVED: process_no_certificate_RESERVED(connection)
		case bad_certificate: process_bad_certificate(connection)
		case unsupported_certificate: process_unsupported_certificate(connection)
		case certificate_revoked: process_certificate_revoked(connection)
		case certificate_expired: process_certificate_expired(connection)
		case certificate_unknown: process_certificate_unknown(connection)
		case illegal_parameter: process_illegal_parameter(connection)
		case unknown_ca: process_unknown_ca(connection)
		case access_denied: process_access_denied(connection)
		case decode_error: process_decode_error(connection)
		case decrypt_error: process_decrypt_error(connection)
		case export_restriction_RESERVED: process_export_restriction_RESERVED(connection)
		case protocol_version: process_protocol_version(connection)
		case insufficient_security: process_insufficient_security(connection)
		case internal_error: process_internal_error(connection)
		case user_canceled: process_user_canceled(connection)
		case no_renegotiation: process_no_renegotiation(connection)
		case unsupported_extension: process_unsupported_extension(connection) /* new */
		case UNKNOWN: process_UNKNOWN(connection)
		}
	}
}
